    ...
    try {
        ...
        fields = fields.map(f => {
            ...
            ACTIVITIES.forEach(a => {
                ...
                // New structure for visual progress polygons per activity.
                if (!Array.isArray(f.activities[a.id].progressPolygons)) {
                    f.activities[a.id].progressPolygons = [];
                }

                // Backward compatibility: derive endTime when only hours exists.
                f.activities[a.id].sessions = f.activities[a.id].sessions.map(sess => {
                    const start = sess.startTime || '07:00';
                    const hoursNum = parseFloat(sess.hours) || 0;
                    if (!sess.endTime) {
                        sess.endTime = addHoursToTime(start, hoursNum);
                    }
                    // Keep legacy field for dashboard compatibility.
                    sess.hours = computeHoursFromStartEnd(start, sess.endTime);
                    return sess;
                });
            });
            return f;
...
function addHoursToTime(startTime, hours) {
    const s = parseTimeToMinutes(startTime);
    return minutesToTime(s + ((parseFloat(hours) || 0) * 60));
}

function calculateVisualOverlap(fieldPolygon, activityPolygons) {
    if (!window.turf || !Array.isArray(fieldPolygon) || fieldPolygon.length < 4) {
        return { acres: 0, percent: 0 };
    }
    const fieldFeature = turf.polygon([fieldPolygon.map(([lat, lng]) => [lng, lat])]);
    let merged = null;
    (activityPolygons || []).forEach(poly => {
        if (!Array.isArray(poly?.polygon) || poly.polygon.length < 4) return;
        const candidate = turf.polygon([poly.polygon.map(([lat, lng]) => [lng, lat])]);
        const clipped = turf.intersect(fieldFeature, candidate);
        if (!clipped) return;
        if (!merged) merged = clipped;
        else {
            try {
                merged = turf.union(merged, clipped) || merged;
            } catch {
                // Keep previous union if turf fails for an edge case.
            }
        }
    });
    if (!merged) return { acres: 0, percent: 0 };
    const overlapAcres = turf.area(merged) / 4046.85642;
    const fieldAcres = calculateAcres(fieldPolygon);
    const percent = fieldAcres > 0 ? Math.min(100, (overlapAcres / fieldAcres) * 100) : 0;
    return {
        acres: parseFloat(overlapAcres.toFixed(2)),
        percent: parseFloat(percent.toFixed(1))
    };
}

function polygonToSvgPath(polygon, width = 76, height = 52, pad = 4) {
    ...
    const sx = (width - pad * 2) / lngSpan;
    const sy = (height - pad * 2) / latSpan;
    return pts.map((p, idx) => {
        const x = pad + ((p[1] - minLng) * sx);
        const y = height - pad - ((p[0] - minLat) * sy);
...
// ===== ACTIVITY VISUAL MAP (SEPARATE FROM MAIN MAP) =====
let activityMap = null;
let activityMapLayers = [];
const activityVisualState = {
    fieldId: null,
    actId: null,
    drawing: false,
    draftPoints: [],
...
function getFieldById(fieldId) {
    return loadFields().find(f => f.id === fieldId);
}

function getActivityRef(field, actId) {
    if (!field.activities[actId]) field.activities[actId] = { sessions: [], progressPolygons: [] };
    if (!Array.isArray(field.activities[actId].progressPolygons)) field.activities[actId].progressPolygons = [];
    return field.activities[actId];
}

function updateActivityVisualCounters() {
    const ptsEl = document.getElementById('activity-poly-points');
    if (ptsEl) ptsEl.textContent = String(activityVisualState.draftPoints.length);
    const drawBtn = document.getElementById('btn-act-draw');
    if (drawBtn) drawBtn.textContent = activityVisualState.drawing ? 'Stop Drawing' : 'Start Drawing';
    const status = document.getElementById('activity-poly-status');
    if (status) status.textContent = `Drawing: ${activityVisualState.drawing ? 'active' : 'paused'}`;

    const field = getFieldById(activityVisualState.fieldId);
    if (!field) return;
    const activity = getActivityRef(field, activityVisualState.actId);
    const visual = calculateVisualOverlap(field.polygon, activity.progressPolygons);
    const acresEl = document.getElementById('activity-poly-acres');
    const fillEl = document.getElementById('activity-poly-fill');
    if (acresEl) acresEl.textContent = visual.acres.toFixed(2);
    if (fillEl) fillEl.textContent = `${visual.percent}%`;
}

function clearActivityMapLayers() {
    if (!activityMap) return;
    activityMapLayers.forEach(l => activityMap.removeLayer(l));
    activityMapLayers = [];
}

function renderActivityPolyList(field, activity) {
    const listEl = document.getElementById('activity-poly-list');
    if (!listEl) return;
    const items = activity.progressPolygons || [];
    if (!items.length) {
        listEl.innerHTML = '<p class="text-xs text-gray-500">No visual polygons saved yet.</p>';
        return;
    }
    listEl.innerHTML = items.map((poly, idx) => `
        <div class="bg-white border rounded px-2 py-1.5 flex items-center justify-between">
            <span class="text-xs text-gray-700">Polygon ${idx + 1}</span>
            <div class="flex items-center gap-2">
                <button type="button" onclick="editActivityPolygon(${idx})" class="text-xs font-semibold text-blue-700 hover:text-blue-900">Edit</button>
                <button type="button" onclick="deleteActivityPolygon(${idx})" class="text-xs font-semibold text-red-700 hover:text-red-900">Delete</button>
            </div>
        </div>
    `).join('');
}

function renderActivityMapModal() {
    if (!activityMap || activityVisualState.fieldId == null || !activityVisualState.actId) return;
    clearActivityMapLayers();
    const field = getFieldById(activityVisualState.fieldId);
    if (!field) return;
    const activity = getActivityRef(field, activityVisualState.actId);

    const fieldPoly = L.polygon(field.polygon, { color: '#111827', weight: 2, fillOpacity: 0.04 }).addTo(activityMap);
    activityMapLayers.push(fieldPoly);

    (activity.progressPolygons || []).forEach((poly, idx) => {
        if (!Array.isArray(poly.polygon) || poly.polygon.length < 4) return;
        const p = L.polygon(poly.polygon, { color: '#2563eb', weight: 2, fillColor: '#60a5fa', fillOpacity: 0.26 }).addTo(activityMap);
        p.bindTooltip(`Saved ${idx + 1}`);
        activityMapLayers.push(p);
    });

    activityVisualState.draftPoints.forEach((pt, idx) => {
        const mk = L.marker(pt, {
            draggable: true,
            icon: L.divIcon({
                className: 'act-draft-vertex',
                html: '<div style="width:16px;height:16px;border-radius:9999px;background:#f97316;border:2px solid #c2410c;"></div>',
                iconSize: [16, 16],
                iconAnchor: [8, 8]
            })
        }).addTo(activityMap);
        mk.bindTooltip(String(idx + 1), { permanent: true, direction: 'top', offset: [0, -8] });
        mk.on('dragend', () => {
            const ll = mk.getLatLng();
            activityVisualState.draftPoints[idx] = [ll.lat, ll.lng];
            renderActivityMapModal();
        });
        activityMapLayers.push(mk);
    });

    if (activityVisualState.draftPoints.length >= 2) {
        activityMapLayers.push(L.polyline(activityVisualState.draftPoints, { color: '#f97316', dashArray: '4,4' }).addTo(activityMap));
    }
    if (activityVisualState.draftPoints.length >= 3) {
        const closed = [...activityVisualState.draftPoints, activityVisualState.draftPoints[0]];
        activityMapLayers.push(L.polygon(closed, { color: '#ea580c', fillColor: '#fb923c', fillOpacity: 0.25 }).addTo(activityMap));
    }

    renderActivityPolyList(field, activity);
    updateActivityVisualCounters();
}

function handleActivityMapClick(e) {
    if (!activityVisualState.drawing) return;
    if (!e?.latlng) return;
    activityVisualState.draftPoints.push([e.latlng.lat, e.latlng.lng]);
    renderActivityMapModal();
}

function openActivityMapModal(fieldId, actId) {
    const modal = document.getElementById('activity-map-modal');
    if (!modal) return;
    activityVisualState.fieldId = fieldId;
    activityVisualState.actId = actId;
    activityVisualState.drawing = false;
    activityVisualState.draftPoints = [];
    activityVisualState.editIndex = null;
    modal.classList.remove('hidden');

    const field = getFieldById(fieldId);
    const actName = ACTIVITIES.find(a => a.id === actId)?.name || actId;
    const title = document.getElementById('activity-map-title');
    if (title) title.textContent = `Activity Visual Progress - ${field?.name || 'Field'} / ${actName}`;

    if (!activityMap) {
        activityMap = L.map('activity-map').setView([36.6, -121.3], 10);
        L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', { maxZoom: 19 }).addTo(activityMap);
        L.tileLayer('https://{s}.basemaps.cartocdn.com/light_only_labels/{z}/{x}/{y}{r}.png', { maxZoom: 19, pane: 'overlayPane' }).addTo(activityMap);
        activityMap.on('click', handleActivityMapClick);
    }
    setTimeout(() => {
        activityMap.invalidateSize();
        if (field?.polygon?.length) {
            const bounds = L.latLngBounds(field.polygon);
            activityMap.fitBounds(bounds, { padding: [30, 30], maxZoom: 17 });
        }
        renderActivityMapModal();
    }, 50);
}
window.openActivityMapModal = openActivityMapModal;

function closeActivityMapModal() {
    document.getElementById('activity-map-modal')?.classList.add('hidden');
    activityVisualState.drawing = false;
    activityVisualState.draftPoints = [];
    activityVisualState.editIndex = null;
}
window.closeActivityMapModal = closeActivityMapModal;

function toggleActivityPolyDrawing() {
    activityVisualState.drawing = !activityVisualState.drawing;
    if (activityVisualState.drawing && activityVisualState.editIndex == null) {
        activityVisualState.draftPoints = [];
    }
    updateActivityVisualCounters();
}
window.toggleActivityPolyDrawing = toggleActivityPolyDrawing;

function undoActivityPolyPoint() {
    if (!activityVisualState.draftPoints.length) return;
    activityVisualState.draftPoints.pop();
    renderActivityMapModal();
}
window.undoActivityPolyPoint = undoActivityPolyPoint;

function clearActivityPolyDraft() {
    activityVisualState.draftPoints = [];
    activityVisualState.editIndex = null;
    renderActivityMapModal();
}
window.clearActivityPolyDraft = clearActivityPolyDraft;

function saveActivityPolyDraft() {
    const pts = activityVisualState.draftPoints;
    ...
    const fields = loadFields();
    const field = fields.find(f => f.id === activityVisualState.fieldId);
    if (!field) return;
    const activity = getActivityRef(field, activityVisualState.actId);

    const record = {
        id: activityVisualState.editIndex != null ? (activity.progressPolygons[activityVisualState.editIndex]?.id || Date.now()) : Date.now(),
        polygon: closed,
        updatedAt: nowIso()
    };
    if (activityVisualState.editIndex != null) {
        activity.progressPolygons[activityVisualState.editIndex] = record;
    } else {
        activity.progressPolygons.push(record);
    }
    saveFields(fields);
    activityVisualState.draftPoints = [];
    activityVisualState.editIndex = null;
...
window.saveActivityPolyDraft = saveActivityPolyDraft;

function editActivityPolygon(idx) {
    const field = getFieldById(activityVisualState.fieldId);
    if (!field) return;
    const activity = getActivityRef(field, activityVisualState.actId);
    const poly = activity.progressPolygons[idx];
    if (!poly?.polygon?.length) return;
    activityVisualState.editIndex = idx;
    activityVisualState.draftPoints = poly.polygon.slice(0, -1).map(p => [p[0], p[1]]);
    activityVisualState.drawing = true;
    renderActivityMapModal();
}
window.editActivityPolygon = editActivityPolygon;

function deleteActivityPolygon(idx) {
    if (!confirm('Delete this visual polygon?')) return;
    const fields = loadFields();
    const field = fields.find(f => f.id === activityVisualState.fieldId);
    if (!field) return;
    const activity = getActivityRef(field, activityVisualState.actId);
    activity.progressPolygons.splice(idx, 1);
    saveFields(fields);
    renderActivityMapModal();
    renderAll();
}
window.deleteActivityPolygon = deleteActivityPolygon;
                        ...
                        <div class="p-3 border-t bg-gray-50/50" data-act-input="${act.id}" data-field-id="${field.id}">
                            <div class="space-y-2">
                                ...
                                <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                                    ...
                                    <div>
                                        <label class="block text-[10px] font-semibold text-gray-500 uppercase tracking-wide mb-1">End</label>
                                        <input type="time" class="act-end w-full text-sm border-2 border-gray-300 rounded-lg px-3 py-2.5 focus:border-ranch-green" value="08:00">
                                    </div>
                                    <div>
                                ...
                                <div class="bg-blue-50 border border-blue-100 rounded-lg p-2 flex items-center justify-between">
                                    <span class="text-xs text-blue-700">Visual fill: <b>${visual.percent}%</b> (${visual.acres} ac)</span>
                                    <button type="button" onclick="openActivityMapModal(${field.id},'${act.id}')" class="text-xs font-semibold text-blue-700 hover:text-blue-900">Open Visual Map</button>
                                </div>
                            ...
                            ${(activity.sessions || []).map((sess, idx) => `
                                <div class="bg-white border border-gray-200 rounded-lg p-2.5">
                                    <div class="flex items-center justify-between mb-1">
                                        <span class="text-xs text-gray-500">Date ${sess.date || '--'} · ${sess.startTime || '--:--'}-${sess.endTime || '--:--'}</span>
                                    ...
                                    <div class="flex items-center justify-between">
                                        ...
                                        <span class="text-sm font-bold text-ranch-green">${(parseFloat(sess.hours) || 0).toFixed(2)}h · ${sess.acres}ac</span>
                                    </div>
            ...
            <div id="field-card-${field.id}" class="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-all">
                <div class="bg-white p-6 text-center border-b">
                    ...
                    <div class="mt-2 flex justify-center">
                        <svg width="76" height="52" viewBox="0 0 76 52" class="block">
                            <path d="${polygonToSvgPath(field.polygon)}" fill="rgba(37,99,235,.20)" stroke="#1d4ed8" stroke-width="1.5"></path>
                        </svg>
                    </div>
                </div>
...
function addSession(fieldId, actId) {
    ...
    const endTime = container.querySelector('.act-end').value || startTime;
    const hours = computeHoursFromStartEnd(startTime, endTime);
    const acres = parseFloat(container.querySelector('.act-acres').value) || 0;
    if (hours <= 0) {
        alert('End time must be after start time. Overnight sessions are not allowed.');
        return;
    }
    if (acres === 0) {
        alert('Enter acres for this session.');
        return;
    }
        ...
        ACTIVITIES.forEach(a => {
            ...
            act.sessions.forEach(sess => {
                ...
                if (workersInExistingSession.length > 0) {
                    const existingStartMin = parseTimeToMinutes(sess.startTime || '00:00');
                    const existingEndMin = parseTimeToMinutes(sess.endTime || addHoursToTime(sess.startTime || '00:00', parseFloat(sess.hours) || 0));
    ...
    field.activities[actId].sessions.push({ date, workers: workersSessionList, hours, acres, startTime, endTime, comments });
    saveFields(fields);
    renderAll();
}
    ...
    fieldMarkers = [];
        fields.forEach(field => {
            ...
            const label = L.marker([(field.swLat + field.neLat) / 2, (field.swLng + field.neLng) / 2], {
                icon: L.divIcon({ className: 'field-label', html: `<div style="background:${color};color:white;padding:6px 10px;border-radius:10px;font-weight:800;font-size:13px;text-align:center;min-width:80px;max-width:150px;box-shadow:0 2px 6px rgba(0,0,0,0.3);font-family:Inter,sans-serif;line-height:1.3;cursor:pointer;" onclick="scrollToField(${field.id})">${field.name}<br>${progress}%<br>${fieldAcres} ac</div>`, iconSize: [140, 60], iconAnchor: [70, 30] })
            }).addTo(map);
